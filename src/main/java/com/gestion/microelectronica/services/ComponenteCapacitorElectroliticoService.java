...
import com.gestion.microelectronica.exceptions.capacitor.electrolitico.CapElectrIdMismatchException;
...
public class ComponenteCapacitorElectroliticoService {
	...
	public void addCapacitorElectrolitico(ComponenteCapacitorElectroliticoEntity capacitorElectrolitico) {
		try {
			if (capacitorElectrolitico == null) {
				...
				throw new CapElectrNotFoundException("EL CAPACITOR ELECTROLITICO ES NULO");
			} else {
			...
			throw new CapElectrNotFoundException("NO SE PUDO INSERTAR EL CAPACITOR ELECTROLITICO ", e, false,
					true);
		}
	}
	...
	public void updateCapacitorElectrolitico(ComponenteCapacitorElectroliticoEntity capacitorElectrolitico) {
		try {
			...
			throw new CapElectrNotFoundException("NO SE PUDO ACTUALIZAR EL CAPACITOR ELECTROLITICO ", e, false,
					true);
		}
	}
	...
	public void deleteCapacitorElectrolitico(int id) {
		try {
			if (id <= 0 || iCapacitorElectroliticoRepository.findById(id).getId() != id) {
				...
				throw new CapElectrIdMismatchException("EL ID NO EXISTE EN LA DB");
			} else {
			...
			throw new CapElectrNotFoundException("NO SE PUDO ELIMINAR EL CAPACITOR ELECTROLITICO ", e, false,
					true);
		}
	}
